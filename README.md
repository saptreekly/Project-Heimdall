...
| **NLP** | Lexicon v2.4 (+ inflammatory escalation bridge) → outrage index, polarity, escalation tier, component scores; optional Hugging Face twitter-roberta sentiment |
...
| `SNAPSHOT_SENTIMENT_STRICT` | Fail `verify_snapshot.py` if sentiment fields or model version are missing/stale |
...
**Sentiment automation (GitHub Actions):** ingest and export workflows set `USE_EMBEDDING_THEMES=true` and `RESCORE_BEFORE_EXPORT=true`, so each run rescores stale posts to the current lexicon (`heimdall-lexicon-v2.4+embed-cluster`) before writing `snapshot.json`. Weekly **maintenance** rescores + re-exports and runs a TweetEval benchmark sample. **daily-analytics** updates `data/dashboard/SENTIMENT_WATCHLIST.md` and opens `sentiment-alert` issues on WoW/trend crossings.
